    ...
    public class RegisterTripValidator : AbstractValidator<RequestRegisterTripJson>
    {
        public RegisterTripValidator()
        {
            ...
            RuleFor(request => request.StartDate.Date)
                ...
                .WithMessage(ResourceErrorMessages.START_DATE_TRIP_MUST_BE_GREATER_THAN_TODAY);

            //RuleFor(request => request.EndDate.Date)
            //    .GreaterThan(request => request.StartDate.Date)
            //    .WithMessage(ResourceErrorMessages.END_DATE_TRIP_MUST_BE_GREATER_THAN_START_DATE);
